...
extend = (term) => {
  ...
  term.init = (user = "guest", preserveHistory = false) => {
    ...
    term.printLogoType();
    term.stylePrint(
      "Welcome to Derek Deming's terminal portfolio.",
    );
    term.stylePrint(
      `Type ${colorText("help", "command")} for commands. ${colorText("whois derek", "command")} to learn about me.`,
      false,
    );
    term.stylePrint(
      `\r\nExplore: ${colorText("skills", "command")}, ${colorText("experience", "command")}, ${colorText("projects", "command")}, or ${colorText("contact", "command")}`,
      false,
    );
